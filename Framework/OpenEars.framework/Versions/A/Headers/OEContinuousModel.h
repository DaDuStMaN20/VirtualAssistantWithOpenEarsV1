...
- (void) removeCmnPlist;
- (void)  testFileChange;
@property (NS_NONATOMIC_IOSONLY, readonly, copy) NSError *stopListening;
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL openEarsLoggingIsOn;
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL verbosePocketsphinxIsOn;
- (void) heartBeat;
- (ps_decoder_t *) initializeDecoderForLanguageModelAtPath:(NSString *)languageModelPath dictionaryAtPath:(NSString *)dictionaryPath acousticModelAtPath:(NSString *)acousticModelPath languageModelIsJSGF:(BOOL)languageModelIsJSGF usingBestpath:(NSNumber *)usingBestpath NS_RETURNS_INNER_POINTER;
...
@property (NS_NONATOMIC_IOSONLY, readonly) NSInteger startUtterance;
- (void) processRaw:(NSData *)buffer;
@property (NS_NONATOMIC_IOSONLY, getter=getInSpeech, readonly) NSInteger inSpeech;
- (void) endUtterance;
...
- (char const *) getHypothesisFromDecoder:(ps_decoder_t *)pocketSphinxDecoder withScore:(SInt32 *)recognitionScore NS_RETURNS_INNER_POINTER;
- (SInt32) getProbabilityFromDecoder:(ps_decoder_t *)pocketSphinxDecoder;
@property (NS_NONATOMIC_IOSONLY, readonly) ps_nbest_t *nBest;
- (ps_nbest_t *)nBestNext:(ps_nbest_t *)nbest NS_RETURNS_INNER_POINTER;
- (char const *)nBestHypothesis:(ps_nbest_t *)nbest withScore:(int32 *)recognitionScore NS_RETURNS_INNER_POINTER;
- (void) nBestFree:(ps_nbest_t *)nbest;
- (char const *) searchHyp:(ps_decoder_t *)ps bestScore:(int *)out_best_score final:(int *)final NS_RETURNS_INNER_POINTER;
